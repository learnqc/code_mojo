...
fn main() raises:
    ...
    assert_almost_equal(state[1].re, 1.0)
    print("Test 1 (QFT + IQFT = Identity) passed.")

    # 2. Test QFT factory
    var qft_qc = QFT(2)
    var qft_state = run_circuit[2](qft_qc)
    # QFT on |00> should give 1/2(|00> + |01> + |10> + |11>)
    # amplitude should be 0.5
    for i in range(4):
        assert_almost_equal(qft_state[i].re, 0.5)
    ...
    var rev_state = run_circuit[2](qc_rev)
    # Should still be back to |00>
    assert_almost_equal(rev_state[0].re, 1.0)
