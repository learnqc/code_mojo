...
fn test_iqft_circuit() raises:
    ...
    # Create state with same initial conditions
    var state2 = QuantumState(n)
    state2[3] = Amplitude(0.707, 0)
    state2[7] = Amplitude(0.707, 0)

    # Execute circuit on state
    from butterfly.core.circuit import execute

    execute(state2, circuit)

    # Compare
    for i in range(1 << n):
        assert_almost_equal(state1[i].re, state2[i].re)
        assert_almost_equal(state1[i].im, state2[i].im)
