...
from butterfly.core.gates import P, X, is_bit_not_set


fn inversion_0_circuit(n: Int) -> QuantumCircuit:
    """
    Reflection-about-zero circuit.
    Logic: X(all) -> MCP(pi) -> X(all)
    """
...
fn diffuser_circuit(n: Int) -> QuantumCircuit:
    ...
    """
    var qc = QuantumCircuit(n)
    var q = qc.add_register("q", n)

    for i in range(n):
        qc.h(i)

    qc.append_circuit(inversion_0_circuit(n), q)
...
fn grover_iterate_circuit(oracle: QuantumCircuit) -> QuantumCircuit:
    ...
    # oracle
    qc.append_circuit(oracle, q)

    # diffusion
    qc.append_circuit(diffuser_circuit(n), q)
...
fn grover_circuit(
    ...
    var qc = QuantumCircuit(num_qubits)
    var q = qc.add_register("q", num_qubits)
...
fn append_grover_to_register(
    ...
    for _ in range(iterations):
        qc.append_circuit(step, register)
