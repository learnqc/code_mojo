...
fn apply_cft(
    ...
    """
    var n = state.size()
    var n_qubits = Int(log2(Float64(n)))

    # v4 seems to have issues or is unoptimized for very small N (e.g. N=16) in the current test configuration.
    # We restrict delegation to N >= 32 (5 qubits) where v4 is known to be robust/faster.
    # [Validation Update]: v4 failed validation for N=32 as well (Diff Sum ~19.5).
    # Disabling delegation until v4 correctness is verified/fixed.
    if (
        False
        and len(targets) == n_qubits
        and n >= 32
        and not inverse
        and do_swap
    ):
        ...
        apply_cft_v5_contiguous(state, targets, inverse, do_swap)
